...
public class SensorRecordProcessor implements IRecordProcessor {
  ...
  private void processSingleRecord(Record record) {
    ...
    try {
      dataFromRecord = getRecordData(record);
      ...
      System.out.printf(
          "Processing single record sensor id: %s, temperature: %d. %n", sensorId, temperature);
      ...
      System.out.println(
          "Record does not match application record format. Ignoring record with data; " + data);
   ...
   *
   * @param record    Name of the Kinesis stream
