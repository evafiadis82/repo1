...
public class Utils {
  ...
  public static final String[] STUDENT_BUCKET_DATA_FILE_KEYS = {
    "DrugAdverseEvents_September.txt", "DrugAdverseEvents_October.txt"
  };
  ...
  public static void setup(
      AmazonS3Client s3ClientForStudentBuckets, String inputBucketName, String outputBucketName)
      throws Exception {
    setupInputBucket(s3ClientForStudentBuckets, inputBucketName);
    setupOutputBucket(s3ClientForStudentBuckets, outputBucketName);
  }

  // Create the input bucket. Copy the CSV-formatted files from the lab bucket to the student's input bucket.
  private static void setupInputBucket(
      AmazonS3Client s3ClientForStudentBuckets, String inputBucketName) throws Exception {
    S3Object sampleDataObject = null;

    // Create a separate instance of the AmazonS3Client to access the lab bucket.
    AmazonS3Client s3ForLabBucket = new AmazonS3Client();
    ...
    if (!s3ClientForStudentBuckets.doesBucketExist(inputBucketName)) {
    ...
    for (int i = 0; i < LAB_BUCKET_DATA_FILE_KEYS.length; i++) {
      ...
      s3ClientForStudentBuckets.putObject(
          inputBucketName,
          STUDENT_BUCKET_DATA_FILE_KEYS[i],
          sampleDataObject.getObjectContent(),
          objectMetadata);
    }
  }

  // Create the output bucket
  // If a bucket with the name already exists, make sure that you have permissions to access the bucket
  private static void setupOutputBucket(
      AmazonS3Client s3ClientForStudentBuckets, String outputBucketName) throws Exception {
    if (!s3ClientForStudentBuckets.doesBucketExist(outputBucketName)) {
      ...
      s3ClientForStudentBuckets.deleteObjects(
          new DeleteObjectsRequest(outputBucketName)
  ...
  public static void verifyBucketOwnership(
      AmazonS3Client s3ClientForStudentBuckets, String bucketName) throws Exception {
    ...
    if (!ownedByYou) {
      String msg =
          String.format(
              "The %s bucket is owned by another account. Specify a unique name for your bucket",
              bucketName);
